...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    tag_pos_adjusted_z = np.copy(tag_pos)
    tag_pos_adjusted_z[0][2] = current_position[2]

    print("Tag Position Adjusted:", tag_pos_adjusted_z)

    if task == 'line':
        trajectory = LinearTrajectory(current_position, tag_pos_adjusted_z, 15)
    elif task == 'circle':
        trajectory = CircularTrajectory(15, tag_pos_adjusted_z, 0.1)
    elif task == 'polygon':
...
def get_controller(controller_name, limb, kin):
    ...
    if controller_name == 'workspace':
        # YOUR CODE HERE
        Kp = np.array([0.1, 0.1, 0.1, -0.1, 0.0, 0.0])
        ...
        # YOUR CODE HERE
        Kp = np.array([0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
        Kv = np.array([0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
