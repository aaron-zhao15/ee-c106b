...
class WorkspaceVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        current_position = get_joint_positions(self._limb)
        pos, orient = target_position[:3], target_position[3:]
        desired_position = self._kin.inverse_kinematics(pos, orient)
        ...
        xi_hat = logm(gtd) # jank log here, might be scipy
        # use numpy version 1.21.0
        xi = vee(xi_hat)

        U_s = self.Kp@xi + Ad_gtd@target_velocity
...
class PDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        position, orientation = target_position[:3], target_position[3:]
        angles = self._kin.inverse_kinematics(position, orientation)

        position_err = target_position - joint_positions
        velocity_err = joint_velocities

        control_input = angles + self.Kp @ position_err + self.Kv @ velocity_err
        self._limb.set_joint_velocities(joint_array_to_dict(control_input, self._limb))
...
class PDJointTorqueController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        positions_dict = joint_array_to_dict(joint_positions, self._limb)
        velocity_dict = joint_array_to_dict(joint_velocities, self._limb)
