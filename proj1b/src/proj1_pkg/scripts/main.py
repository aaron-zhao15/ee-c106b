...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    try:
        ...
        # self._kin.forward_position_kinematics() might be relevant for getting hand transform
    except Exception as e:
        print(e)
    ...
    tag_pos_adjusted_z = np.copy(tag_pos[0])
    tag_pos_adjusted_z[2] = current_position[2]

    polygon_points = [current_position, tag_pos_adjusted_z, current_position]


    print("Tag Position Adjusted:", tag_pos_adjusted_z)

    if task == 'line':
        trajectory = LinearTrajectory(current_position, tag_pos_adjusted_z, 10)
    elif task == 'circle':
        trajectory = CircularTrajectory(current_position, tag_pos_adjusted_z, 0.1, 10, 3)
    elif task == 'polygon':
        trajectory = PolygonalTrajectory(polygon_points, 10)
    else:
...
def get_controller(controller_name, limb, kin):
    ...
    if controller_name == 'workspace':
        # YOUR CODE HERE
        Kp = np.array([0.01, 0.01, 0.01, 0.01, 0.01, 0.01])
        Kv = np.array([0, 0, 0, 0, 0, 0])
        controller = WorkspaceVelocityController(limb, kin, Kp, Kv)
    elif controller_name == 'jointspace':
        # YOUR CODE HERE
        Kp = np.array([1, 1, 1, 0.1, 0.3, 0.1, 0.2])
        Kv = np.array([0.5, 0.5, 0.5, 0.1, 0.05, 0.1, 0.1])
        controller = PDJointVelocityController(limb, kin, Kp, Kv)
    elif controller_name == 'torque':
        # YOUR CODE HERE
        Kp = np.array([40, 150, 80, 90, 85, 25, 50])
        Kv = np.array([1, 1, 1, 0.4, 3, 3, 3])
...
def main():
    ...
    # ik_solver = IK("base", "right_hand") # this IK solver sucks
    ik_solver = None
